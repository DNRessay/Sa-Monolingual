...
This now runs automatically as the last stage of the pipeline — a third
Lambda (**PushFunction**) that Step Functions invokes per language right
after ProcessFunction finishes. It downloads `clean/{lang}.jsonl` from S3
and calls the same `push_lang()` used locally.

The HF write token is passed in as a CloudFormation parameter (`HfToken`,
`NoEcho: true`) and only ever lands as an environment variable on
PushFunction — FetchFunction and ProcessFunction never see it. If you're
deploying by hand rather than through GitHub Actions:

```bash
sam deploy --parameter-overrides "Stage=dev HfToken=hf_xxx"
```

Through GitHub Actions, set `HF_TOKEN` as a repo secret (Settings → Secrets
and variables → Actions) — the `deploy-aws.yml` workflow passes it straight
through to `sam deploy` on your behalf. It's never written to
`samconfig.toml`, so it never ends up committed to the repo.

You can still run `push.py` locally against a synced `data/clean/` directory
exactly as before, if you'd rather review before pushing:
...
- PushFunction runs as a single Lambda invocation per language, same 15-minute
  cap. For Afrikaans-scale row counts this may not fully upload in one call —
  `push_to_hub` shards internally, so a timeout mid-upload is a partial push,
  not a corrupt one; re-running that language's push resumes cleanly.
